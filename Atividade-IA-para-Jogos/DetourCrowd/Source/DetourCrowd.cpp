...
bool dtCrowd::init(const int maxAgents, const float maxAgentRadius, dtNavMesh* nav)
{
	...
	// Explorer
	m_filters[0].setAreaCost(JV_GROUND, JV_AREA_MULTIPLIER/2);
	m_filters[0].setAreaCost(JV_WATER, JV_AREA_MULTIPLIER*10);
	m_filters[0].setAreaCost(JV_ROAD, JV_AREA_MULTIPLIER);
	m_filters[0].setAreaCost(JV_DOOR, JV_AREA_MULTIPLIER);
	m_filters[0].setAreaCost(JV_GRASS, 1.0f);
	m_filters[0].setAreaCost(JV_JUMP, JV_AREA_MULTIPLIER*1.5);
	// Runner
	m_filters[1].setAreaCost(JV_GROUND, JV_AREA_MULTIPLIER/50);
	m_filters[1].setAreaCost(JV_WATER, JV_AREA_MULTIPLIER*10);
	m_filters[1].setAreaCost(JV_ROAD, 1.0f);
	m_filters[1].setAreaCost(JV_DOOR, JV_AREA_MULTIPLIER);
	m_filters[1].setAreaCost(JV_GRASS, JV_AREA_MULTIPLIER);
	m_filters[1].setAreaCost(JV_JUMP, JV_AREA_MULTIPLIER*1.5);
	// Swimmer
	m_filters[2].setAreaCost(JV_GROUND, JV_AREA_MULTIPLIER*2);
	m_filters[2].setAreaCost(JV_WATER, 1.0f);
	m_filters[2].setAreaCost(JV_ROAD, JV_AREA_MULTIPLIER*2);
	m_filters[2].setAreaCost(JV_DOOR, JV_AREA_MULTIPLIER);
	m_filters[2].setAreaCost(JV_GRASS, JV_AREA_MULTIPLIER*10);
	m_filters[2].setAreaCost(JV_JUMP, JV_AREA_MULTIPLIER*1.5);
...
int dtCrowd::addAgent(const float* pos, const dtCrowdAgentParams* params)
{
	...
	dtPolyRef ref = 0;
	dtVcopy(nearest, pos);
	dtStatus status = m_navquery->findNearestPoly(pos, m_agentPlacementHalfExtents, &m_filters[ag->params.type], &ref, nearest);
...
void dtCrowd::updateMoveRequest(const float /*dt*/)
{
	...
	for (int i = 0; i < m_maxAgents; ++i)
	{
		...
		if (ag->targetState == DT_CROWDAGENT_TARGET_REQUESTING)
		{
			...
			m_navquery->initSlicedFindPath(path[0], ag->targetRef, ag->npos, ag->targetPos, &m_filters[ag->params.type]);
	...
	for (int i = 0; i < nqueue; ++i)
	{
		...
		ag->targetPathqRef = m_pathq.request(ag->corridor.getLastPoly(), ag->targetRef,
											 ag->corridor.getTarget(), ag->targetPos, &m_filters[ag->params.type]);
...
void dtCrowd::updateTopologyOptimization(dtCrowdAgent** agents, const int nagents, const float dt)
{
	...
	{
		dtCrowdAgent* ag = queue[i];
		ag->corridor.optimizePathTopology(m_navquery, &m_filters[ag->params.type]);
		ag->topologyOptTime = 0;
	}
...
void dtCrowd::checkPathValidity(dtCrowdAgent** agents, const int nagents, const float dt)
{
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		if (!m_navquery->isValidPolyRef(agentRef, &m_filters[ag->params.type]))
		{
			...
			agentRef = 0;
			m_navquery->findNearestPoly(ag->npos, m_agentPlacementHalfExtents, &m_filters[ag->params.type], &agentRef, nearest);
...
//			ag->corridor.trimInvalidPath(agentRef, agentPos, m_navquery, &m_filter);
		...
		{
			if (!m_navquery->isValidPolyRef(ag->targetRef, &m_filters[ag->params.type]))
			{
				// Current target is not valid, try to reposition.
				float nearest[3];
				dtVcopy(nearest, ag->targetPos);
				ag->targetRef = 0;
				m_navquery->findNearestPoly(ag->targetPos, m_agentPlacementHalfExtents, &m_filters[ag->params.type], &ag->targetRef, nearest);
		...
		if (!ag->corridor.isValid(CHECK_LOOKAHEAD, m_navquery, &m_filters[ag->params.type]))
		{
			// Fix current path.
//			ag->corridor.trimInvalidPath(agentRef, agentPos, m_navquery, &m_filter);
//			ag->boundary.reset();
...
void dtCrowd::update(const float dt, dtCrowdAgentDebugInfo* debug)
{
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		if (dtVdist2DSqr(ag->npos, ag->boundary.getCenter()) > dtSqr(updateThr) ||
			!ag->boundary.isValid(m_navquery, &m_filters[ag->params.type]))
		{
			ag->boundary.update(ag->corridor.getFirstPoly(), ag->npos, ag->params.collisionQueryRange,
								m_navquery, &m_filters[ag->params.type]);
		}
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		ag->ncorners = ag->corridor.findCorners(ag->cornerVerts, ag->cornerFlags, ag->cornerPolys,
												DT_CROWDAGENT_MAX_CORNERS, m_navquery, &m_filters[ag->params.type]);
		...
		if ((ag->params.updateFlags & DT_CROWD_OPTIMIZE_VIS) && ag->ncorners > 0)
		{
			...
			ag->corridor.optimizePathVisibility(target, ag->params.pathOptimizationRange, m_navquery, &m_filters[ag->params.type]);
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		m_navquery->m_nav->getTileAndPolyByRefUnsafe(ag->corridor.getFirstPoly(), &bestTile, &bestPoly);

		dtCrowdAgentParams ap;
		memset(&ap, 0, sizeof(ap));
		ap.maxAcceleration = 8.0f;
		ap.maxSpeed = 3.5f;
		ap.type = ag->params.type;
		ap.radius = ag->params.radius;
		ap.height = ag->params.height;
		ap.collisionQueryRange = ag->params.collisionQueryRange;
		ap.pathOptimizationRange = ag->params.pathOptimizationRange;
		ap.updateFlags = ag->params.updateFlags;
		ap.obstacleAvoidanceType = ag->params.obstacleAvoidanceType;
		ap.separationWeight = ag->params.separationWeight;

		switch (ag->params.type) {
		case 0:
			switch (bestPoly->getArea()) {
			case 4: // SAMPLE_POLYAREA_GRASS
				dtVscale(ag->dvel, dvel, 2.0f);
				ap.maxAcceleration = 16.0f;
				ap.maxSpeed = 5.0f;
				break;
			}
			break;
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		// Move along navmesh.
		ag->corridor.movePosition(ag->npos, m_navquery, &m_filters[ag->params.type]);
