...
inline float dtQueryFilter::getHeuristic(const float* pa, const float* pb,
										 const dtPolyRef /*prevRef*/, const dtMeshTile* prevTile, const dtPoly* prevPoly,
										 const dtPolyRef /*curRef*/, const dtMeshTile* curTile, const dtPoly* curPoly,
										 const dtPolyRef /*nextRef*/, const dtMeshTile* nextTile, const dtPoly* nextPoly) const
{
	//float cos = dtVdot(prevTile->verts, curTile->verts) / dtVlen(prevTile->verts)*dtVlen(curTile->verts);
	//float ang = (float )acos(cos);

	float diff = (prevTile->verts[2]) - (curTile->verts[2]);
	
	if(diff >= 0) {
		return dtVdist(pa, pb)*H_SCALE * diff;
	}
	else {
		return dtVdist(pa, pb)*H_SCALE * -diff;
	}
	...
	while (!m_openList->empty())
	{
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			...
			else
			{
				...
				heuristic = dtVdist(neighbourNode->pos, endPos) * H_SCALE * bestNode->modifierType;
			}
...
dtStatus dtNavMeshQuery::updateSlicedFindPath(const int maxIter, int* doneIters)
{
	...
	while (iter < maxIter && !m_openList->empty())
	{
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			...
			{
				heuristic = m_query.filter->getHeuristic(neighbourNode->pos, m_query.endPos,
														 bestRef, bestTile, bestPoly,
														 neighbourRef, neighbourTile, neighbourPoly,
														 0, 0, 0);
			}
